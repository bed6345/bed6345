...
ลูกสูบดัน/ดึง → ถูก cancel เสมอ ลบได้ทางเดียวคือเจ้าของทุบเอง (เด้ง form ยืนยันกันทุบพลาด)
...
| `config.json` | `max_claims_per_player` (default 3), `border_particle`, `border_seconds` |
...
- การลามของไฟแบบ block-to-block ที่ไม่ผ่าน event ที่ API เปิดให้ดักใน v0.11.4
  (กันเท่าที่ event รองรับ: ของเหลว/ไฟไหลทับศูนย์กลาง, ระเบิด)
